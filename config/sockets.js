...
var cookieParser = require('sails/node_modules/express').cookieParser('c831b46118e8a76b87e5a645b901df68');//(sails.config.session.secret);
var Session = require('barrels/node_modules/sails/lib/session')(sails);



//Abonnement du superviseur aux ressources necessaires
function subscribeSuperviseur(socket, superviseur) {

    // Si évènement en cours abonné le socket à tous les organisateurs (dans un premier temps)
    Evenement.findOne({superviseur: superviseur.id, status: 1}).populate('organisateurs').exec(function (err, evt) {
        if (err) {
            console.log(err);
        } else if (evt) {
            console.log("Superviseur abonné aux Organisateurs et à l'évènemement en cours");
            //Update de la position des organisateurs
            Organisateur.subscribe(socket, evt.organisateurs, ['create', 'update']);
            //Update des participants
            Evenement.subscribe(socket, evt)
        } else {
            console.log('pas d\'evenements en cours');
        }
    })
}

//Abonnement des organisateurs aux ressources necessaires
function subscribeOrganisateur(socket, organisateur){

    Organisateur.findOne(organisateur.id).populate('evenements').exec(function(err, org){
        if(err){
            console.log(err);
        } else if(org){

            //Abonnement de l'organisateur à tous ses évènements pour être notifié de leurs status
            Evenement.subscribe(socket, org.evenements,['create','update']);

            //Parcours de la liste évènements des organisateurs
            for(var i=0; i<org.evenements.length; i++){

                //Si il y a un évènement en cours
                if(org.evenements[i].status==1){
                    Evenement.findOne(org.evenements[i].id).populate('organisateurs').exec(function(err, evt){
                        console.log("Organisateur abonné aux organisateurs de l'événement en cours");
                        Organisateur.subscribe(socket , evt.organisateurs ,['create','update']);
                    });
                }
            }
        } else {
            console.log("Organisateur introuvable");
        }
    })
}

//Desabonnement du socket du superviseur
function unsubscribeSuperviseur(socket, event_id) {
    Evenement.findOne(event_id).populate('organisateurs').exec(function (err, evt) {
        Organisateur.unsubscribe(socket, evt.organisateurs);
        Evenement.unsubscribe(socket, evt);
    })
}
//Desabonnement du socket de l'organisateur
function unsubscribeOrganisateur(socket, event_id) {
    Evenement.findOne(event_id).exec(function (err, evt) {
        Organisateur.unsubscribe(socket,evt.organisateurs);
    })
}

function getCurrentId(socket){
...
module.exports.sockets = {
  ...
  // mixes in socket.io events for your routes ad blueprints automatically.
  onConnect: function(session, socket) {
      console.log("Nombre de sockets")
      console.log(sails.io.sockets.clients().length);
      //Check si il s'agit d'un superviseur ou d'un organisateur
      var superviseur = socket.handshake.user.superviseur_owner;
      var organisateur = socket.handshake.user.staff_owner;

      //Si un évènement est en cours, on abonne les entitées
      if(superviseur) {
          subscribeSuperviseur(socket,superviseur);
      }else if(organisateur){
          console.log("Organisateur connecté via sockets")
          subscribeOrganisateur(socket,organisateur)
      }

      //Lancement d'un évènement-----
      //Superviseur
      socket.on('subscribeSuperviseur', function(data){
          if(superviseur)
              subscribeSuperviseur(socket,superviseur);
      })
      //Organisateur
      socket.on('subscribeOrganisateur', function(data){
          if(organisateur)
            subscribeOrganisateur(socket,organisateur)
      });
      ...
      //Fin d'un évènement--------
      //Organisateur
      socket.on('unsubscribeOrganisateur', function(data){
          if(organisateur)
              unsubscribeOrganisateur(socket, data)
      })
      //Suite a la fin de l'évènement courant
      //Superviseur
      socket.on('unsubscribeSuperviseur', function(data){
          if(superviseur)
              unsubscribeSuperviseur(socket, data.event_id);
      })
